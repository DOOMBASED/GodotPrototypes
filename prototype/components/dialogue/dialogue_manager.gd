			...
			for i: int in range(quest.objectives.size()):
				if quest.objectives[i].id == quest.objectives[i].id:
						if i != 0 and not quest.objectives[i - 1].is_completed:
							return {"text": "", "options": {}}
				if quest.objectives[i].target_id == npc.resource.id and quest.objectives[i].target_type == quest.objectives[i].target_types["talk_to"] and not quest.objectives[i].is_completed:
