...
func set_movement_target() -> void:
	if not target_set:
		target_set = true
		var movement_target: Vector2 = NavigationServer2D.region_get_random_point(Global.worldspace.navigation_region.get_rid(), 1, false)
		set_target_position(movement_target)

func _physics_process(_delta) -> void:
	if Dialogue.dialogue_ui.dialogue.visible:
		return
